...
class WXBizMsgCrypt {
  constructor(config) {
    this.m_sToken = config.token;
    this.m_sEncodingAESKey = config.encodingAESKey;
    this.m_sCorpID = config.appid;
  }
...
function WXBizMsgCrypt(config = {}) {
  if (!config.sToken || !config.sEncodingAESKey || !config.sCorpID) throw new Error("配置不齐全！");
