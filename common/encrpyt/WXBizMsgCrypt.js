...
//						当return返回0时有效
// return：成功0，失败返回对应的错误码
WXBizMsgCrypt.prototype.EncryptMsg=function (sReplyMsg,sTimeStamp,sNonce) {
  if (this.m_sEncodingAESKey.length != 43)
    return {errcode:WXBizMsgCryptErrorCode.WXBizMsgCrypt_IllegalAesKey,sEncryptMsg:""}
  try{
    var raw = Cryptography.AES_encrypt(sReplyMsg, this.m_sEncodingAESKey, this.m_sCorpID);
  }
  catch(err){
    return {errcode:WXBizMsgCryptErrorCode.WXBizMsgCrypt_EncryptAES_Error,sEncryptMsg:""}
  }
  ...
  sEncryptMsg += "</xml>";
  return {errcode:0,sEncryptMsg:sEncryptMsg};
}
